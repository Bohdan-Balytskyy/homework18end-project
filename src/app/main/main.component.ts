...
export class MainComponent implements OnInit {
  ...
  checkInput($event): void {
    ...
    if (this.countInput.length > this.lengthInput) {
      ...
      if (count == 0) this.countInput = this.countInput.slice(0, -1);
      if (count == 0) this.countInput = this.oldCountInput;
    }
  }
  count(str): void {
    try {
    ...
    for (let i = 0; i < countAddAndSub; i++) {
      let reg = /(\d+(?:\.\d+)?)(\+|-)(-?\d+(?:\.\d+)?)/;
      let operation = str.match(reg)[2];
      operation === '+' ? str = str.replace(reg, Number(str.match(reg)[1]) + Number(str.match(reg)[3])) : str = str.replace(reg, str.match(reg)[1] - str.match(reg)[3])
    }
    this.countInput = str;
    }
    catch {
